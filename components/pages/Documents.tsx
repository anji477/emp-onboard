...
const Documents: React.FC = () => {
    ...
    const handleSingleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0] && targetDocId) {
            ...
            setUserDocuments(docs => docs.map(doc => 
                doc.id === targetDocId ? { ...doc, status: DocumentStatus.Uploaded, actionDate: new Date().toISOString().split('T')[0], rejectionReason: undefined } : doc
            ));
        }
            ...
            setUploadingFiles(prev =>
                prev.map(f => {
                    if (f.id === upload.id && !f.error) {
                        ...
                        if (Math.random() < 0.25 && f.progress > 20 && f.progress < 90) {
    ...
    const handleVerification = (docId: string) => {
        setUserDocuments(docs => docs.map(doc => 
            doc.id === docId 
    ...
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-gray-800">Documents Hub</h1>
                <p className="text-gray-600 mt-1">Manage your documents and access important company files.</p>
            </div>

            <Card>
                <h2 className="text-xl font-semibold text-gray-700 mb-4">Your Required Documents</h2>
                <p className="text-sm text-gray-500 mb-6">Please upload these specific documents. Our HR team will review them shortly.</p>
                        ...
                        <div key={doc.id} className="flex items-start justify-between py-4 flex-wrap gap-2">
                            <div className="flex items-start">
                                ...
                                <div className="ml-4">
                                    ...
                                    <p className="text-sm text-gray-500">{doc.status}{doc.actionDate ? ` on ${doc.actionDate}` : ''}</p>
            ...
            <Card>
                <h2 className="text-xl font-semibold text-gray-700 mb-4">Upload Additional Documents</h2>
                 <input 
                    type="file" 
                    multiple 
                ...
                <div 
                    ...
                    className="flex justify-center w-full px-6 py-10 mt-2 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-indigo-500 bg-gray-50 hover:bg-indigo-50 transition-colors"
                >
                ...
                {uploadingFiles.length > 0 && (
                    <div className="mt-6 space-y-4">
                        <h3 className="font-medium text-gray-700">Uploading...</h3>
                        {uploadingFiles.map(upload => (
                            <div key={upload.id}>
                                <div className="flex justify-between items-center text-sm">
                                    <p className="font-medium text-gray-800 truncate pr-4">{upload.file.name}</p>
                                    <p className="text-gray-500">{formatFileSize(upload.file.size)}</p>
                                </div>
            ...
            <Card>
                <h2 className="text-xl font-semibold text-gray-700 mb-4">Company Documents</h2>
                <div className="space-y-3">
                    {mockCompanyDocuments.map(doc => (
                        <a key={doc.id} href={doc.url} download className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
